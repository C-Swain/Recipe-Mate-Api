...
const getRecipesWithSearch = async (request, response) => {
  ...
  ;`, [`%${search}%`],
    (error, results) => {
      if (error) {
        response.status(500).send("Our bad. Something went wrong!")
      } else {
        response.status(200).json(results.rows)
      }
    });
...
//    ON CONFLICT (user_id, category_id, likes) DO
//    UPDATE recipes SET name = $2, category_id = $3, description = $4, ingredients = $5, steps = $6,  servings = $7, time = $8, likes = $9, image = $10 RETURNING *`,
//     [user_id, name, category, description, ingredients, steps, servings, time, likes, image],

//     (error, results) => {
//       if (error) { console.log(error) }
//       response.status(201).send(`Recipe updated successfully.`);
//     });
// };

const updateRecipe = async(request, response) => {

  const { user_id, name, category, description, ingredients, steps, servings, time, likes, image } = request.body;
   ...
   ON CONFLICT (id) DO
    ...
    .then(() => {
      setTimeout(() => {
        ...
        updateRecipe(Number(request.params.id), request.body);
      }, 1000);
    })
